...
export default async function getResumeData() {
  ...
  //  if (!dynamicResumeCol) 
    ...
    //       singleFields: foundSingleFields, skills: foundSkills});
        // Removes mongoose special objectId via the _id field
    foundOtherProjects.forEach((otherProject) => delete otherProject._id);
    foundProjects.forEach((project) => delete project._id);
    foundSingleFields.forEach((singleField) => delete singleField._id);
    foundSkills.forEach((skill) => delete skill._id);

    console.log({
        foundOtherProjects, foundProjects, foundSingleFields, foundSkills
    })
    return {
